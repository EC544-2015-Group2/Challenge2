...
function dataFactory(startDate, endDate) {
    ...
    for (var i = startDate, i < endDate, i += 300000) {
        ...
        deviceList.forEach(function(device, index) {
            var date = new Date(i);
            var val = 15 - 5 * Math.cos(Math.PI / 24 * (date.getHours() + date.getMinutes() / 60)) - 20 * Math.cos(Math.PI / 12 * (date.getMonth() + date.getDate() / 31)) + (Math.random() - 0.5);
